...
from common.vehicle import Behaviour, Vehicle, VehicleType, Action
from mcts import mcts
from mcts.flow_state import FlowState
try:
    from LLMAgent import LLMAgent
except ImportError:
    LLMAgent = None  # Not needed for LLM-Raft pipeline
import logger
from simInfo.Memory import DrivingMemory
from simInfo.EnvDescriptor import EnvDescription
from simModel.CarFactory import egoCar
from abstract_decision_maker import EgoVehDecision
...
class EgoDecisionMaker(AbstractEgoDecisionMaker):
    def make_decision(
        ...
        vehicles_info: dict,
        ego_veh: Vehicle,
        descriptor: EnvDescription = EnvDescription(),
    ) -> EgoVehDecision:
        # 2.1 get current description based on rules
        if special_target is not None:
            navInfo = special_target+"You need to develop a more detailed navigation strategy based on your target and make decisions about your ACTION, ACELERATION & STEERING based on that strategy!"
            special_target = None
        else:
            navInfo = descriptor.getNavigationInfo(roadgraph, vehicles_info)
        actionInfo = descriptor.getAvailableActionsInfo(roadgraph, vehicles_info)
        envInfo = descriptor.getEnvPrompt(roadgraph, vehicles_info)
        # 2.2 make decision by LLM
        agent = LLMAgent(ego_veh,use_memory=False)
        ego_behaviour, response, human_question, fewshot, llm_cost, chosen_acceleration, steering= agent.makeDecision(T,[envInfo, actionInfo, navInfo])

        # descriptor.decision = ego_behaviour # update the last decision
        pass

        
        # The decision is a tuple of (action_id, action_value)
        ego_action = Action(acc=chosen_acceleration,steering=steering)

        pass
        # current_QA = QuestionAndAnswer(envInfo, navInfo, actionInfo, fewshot, response, llm_cost["prompt_tokens"], llm_cost["completion_tokens"], llm_cost["total_tokens"], time.time()-T, ego_behaviour)
        return EgoVehDecision(ego_veh, ego_behaviour, ego_action)
    
    async def amake_decision(
        self,
        T: float,   
        roadgraph: RoadGraph,
        vehicles_info: dict,
        ...
        # 2.1 get current description based on rules
        special_target = "Your target: you need to try your best to overtake all cars around you as far as you can in a safe manner,while doing your best to prevent the vehicles behind you from passing you!"
        if special_target is not None:
            navInfo = special_target+"You need to develop a more detailed navigation strategy based on your target and make decisions about your ACTION, ACELERATION & STEERING based on that strategy!"
            special_target = None
        else:
        ...
        # 2.2 make decision by LLM
        agent = LLMAgent(ego_veh,use_memory=False)
        ego_behaviour, response, human_question, fewshot, llm_cost, chosen_acceleration, steering= await agent.amakeDecision(T,roadgraph,[envInfo, actionInfo, navInfo])
        ...
        pass
        # current_QA = QuestionAndAnswer(envInfo, navInfo, actionInfo, fewshot, response, llm_cost["prompt_tokens"], llm_cost["completion_tokens"], llm_cost["total_tokens"], time.time()-T, ego_behaviour)
        return EgoVehDecision(ego_veh.id, ego_behaviour, ego_action)
