...
from common.vehicle import Behaviour, Vehicle, VehicleType, create_vehicle, create_vehicle_lastseen,Action
...
class TrafficManager:
    ...
    def plan(self, T: float, roadgraph: RoadGraph,
             vehicles_info: dict,
             ego_behaviour: Behaviour = Behaviour(8),
             ego_action:Action = Action(0,0),
             other_plan: bool = True,
             external_multi_decision: MultiDecision | None = None) -> Dict[int, Trajectory]:
        """
        ...
        # print("history tracks:", history_tracks)
        ...
        for vehicle_id, vehicle in vehicles.items():
            ...
            if vehicle.vtype == VehicleType.EGO:
                try:
                    ...
                    logging.error(f"Error when updating behaviour of vehicle {vehicle_id}: {e}")
                    vehicle.behaviour = Behaviour.IDLE
                    vehicle.available_lanes = set()
        # Prediction Module
        ...
        if external_multi_decision is not None:
            self.mul_decisions = external_multi_decision
            self.last_decision_time = T
        elif self.config["USE_DECISION_MAKER"] and T - self.last_decision_time >= self.config["DECISION_INTERVAL"]:
            if self.config["EGO_PLANNER"]:
        ...
        vehicles[ego_id].action = ego_action
        
        ego_path = self.ego_planner.plan(vehicles[ego_id], 
                                        observation,
                                        roadgraph, 
                                        prediction, 
                                        T,
    ...
    def extract_history_tracks(self, current_time_step: int,
                            vehicles: Dict[int, Vehicle]) -> Dict[int, List[State]]:
        history_tracks = {}
        for vehicle_id, vehicle in vehicles.items():
            # Check if vehicle has a trajectory and the trajectory has states
            if vehicle_id in self.lastseen_vehicles and vehicle.trajectory and vehicle.trajectory.states:
                # Adjust index based on the current time step and the length of the trajectory states
                index_start = max(0, len(vehicle.trajectory.states) - (current_time_step - self.time_step))
                history_tracks[vehicle_id] = vehicle.trajectory.states[index_start:]
        # print(history_tracks)
    ...
    def extract_vehicles(
        ...
        for vehicle in vehicles_info["carInAoI"]:
            ...
            len(self.lastseen_vehicles[vehicle["id"]].trajectory.states) > through_timestep:
                last_state = self.lastseen_vehicles[vehicle["id"]].trajectory.states[through_timestep]
                vehicles[vehicle["id"]] = create_vehicle_lastseen(
                    vehicle,
                    self.lastseen_vehicles[vehicle["id"]],
                    roadgraph,
