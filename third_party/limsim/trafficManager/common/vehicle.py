...
class Action:
    def __init__(self, acc, steering):
        ...
        steering: steering angle, degree
        """
        self.acc = acc
        self.steering = steering * math.pi / 180
        ...
        if self.current_state.s > current_lane.course_spline.s[-1] - 0.2:
            if isinstance(current_lane, NormalLane):
                ...
                if next_lane is None:
                    logging.error(
                        f"Vehicle {self.id} cannot switch to the next lane, "
                        f"current lane {self.lane_id}, available lanes {self.available_lanes}"
                    )
                    return
