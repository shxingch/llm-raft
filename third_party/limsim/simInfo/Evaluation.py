...
class Score_List(list):
    ...
    def eval_score(self, hyper_parameter: Hyper_Parameter)-> dict:
        ...
        return {'driving score':(hyper_parameter.score_weight["comfort"] * comfort + hyper_parameter.score_weight["efficiency"] * efficiency + hyper_parameter.score_weight["safety"] * safety) * red_light * speed_limit_penalty * self.penalty * 100, 'comfort': comfort, 'efficiency': efficiency, 'safety': safety, 'speed_limit': speed_limit_penalty}
...
class Decision_Evaluation:
    ...
    def SaveResultinDB(self, model: ReplayModel) -> None:
        ...
        cur.execute(
            ...
            (
                self.final_s, self.complete_percentage, self.decision_score.eval_score(self.hyper_parameter)['driving score'], model.sr.ego.id
            )
        )
    ...
    def Evaluate(self, model: ReplayModel) -> None:
        ...
        if model.tpEnd:
            ...
            score_result = self.decision_score.eval_score(self.hyper_parameter)
            decision_score = round(score_result['driving score'],3)
            ...
            # the driving score
            self.logger.info("your driving score is {}".format(decision_score))
            self.logger.info("your driving time is {} s".format((model.timeStep - self.current_time)/10))
            score_result['final score'] = self.final_s
            score_result['driving mile'] = round(self.driving_mile, 3)
            score_result['route length'] = round(self.route_length, 3)
            score_result['complete percentage'] = round(self.complete_p*100, 3)
            score_result['driving time'] = (model.timeStep - self.current_time)/10
        return score_result
        
    @property
    def final_s(self) -> float:
        self.final_score = self.decision_score.eval_score(self.hyper_parameter)['driving score'] * self.complete_p
        # print("final score is ", self.final_score)
        return self.final_score
    
    @property
    def complete_p(self) -> float:
        # 存在error，会导致route_length为0,故当route_length为0时，将其设置为0.01以使得complete_percentage不会出现除零错误
        if self.route_length == 0.0:
            self.route_length = 0.01
